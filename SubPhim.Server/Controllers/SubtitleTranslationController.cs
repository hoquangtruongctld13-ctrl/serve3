...
namespace SubPhim.Server.Controllers
{
    ...
    [ApiController]
    [Route("api/subtitle")]
    [Authorize] // Yêu cầu xác thực JWT cho tất cả endpoints
    public class SubtitleTranslationController : ControllerBase
    {
        ...
        public async Task<IActionResult> Translate([FromBody] TranslateRequest request)
        {
            try
            {
                // Lấy userId từ JWT token để tính lượt dịch
                var userIdString = User.FindFirstValue("id");
                if (!int.TryParse(userIdString, out int userId))
                {
                    return Unauthorized(new ErrorResponse { Error = "Token không hợp lệ hoặc đã hết hạn" });
                }
                ...
                _logger.LogInformation("Received translation request from User {UserId}: {SessionId}, {Lines} lines",
                    userId, request.SessionId, request.Lines.Count);
                ...
                // Truyền userId để tính lượt dịch LocalSRT
                var result = await _orchestratorService.SubmitJobAsync(serviceRequest, userId: userId);
        ...
        /// </summary>
        /// <remarks>
        /// Endpoint này được gọi bởi các server phân tán, không yêu cầu xác thực JWT
        /// </remarks>
        [HttpPost("callback/{sessionId}/{batchIndex}")]
        [AllowAnonymous] // Cho phép server phân tán gọi mà không cần JWT
        ...
        /// </summary>
        /// <remarks>
        /// Endpoint công khai để kiểm tra trạng thái server
        /// </remarks>
        [HttpGet("health")]
        [AllowAnonymous] // Health check không yêu cầu xác thực
