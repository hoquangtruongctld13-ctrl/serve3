...
namespace SubPhim.Server.Data
{
    ...
    public class User
    {
        ...
        public long VbeeCharacterLimit { get; set; } = 0; // 0 = không có quyền sử dụng

        [Display(Name = "Số ký tự Vbee đã dùng")]
        public long VbeeCharactersUsed { get; set; } = 0;

        [Display(Name = "Lần cuối reset bộ đếm Vbee (UTC)")]
        public DateTime LastVbeeResetUtc { get; set; } = DateTime.UtcNow;

        [Display(Name = "Reset Vbee quota sau 30 ngày")]
        public bool VbeeResetMonthly { get; set; } = false; // false = cộng dồn, true = reset sau 30 ngày

        [Display(Name = "Ngày bắt đầu tính quota Vbee")]
        public DateTime? VbeeQuotaStartedAt { get; set; } // null = chưa bật, giá trị = ngày bắt đầu tính 30 ngày
    }
    ...
    public class TierDefaultSetting
    {
        ...
        // === Vbee TTS Limits ===
        [Display(Name = "Giới hạn ký tự Vbee TTS")]
        public long VbeeCharacterLimit { get; set; } = 0;

        [Display(Name = "Reset Vbee TTS sau 30 ngày")]
        public bool VbeeResetMonthly { get; set; } = false;
    }
    ...
    public class UpdateInfo
    {
        ...
        [Display(Name = "Bắt buộc cập nhật")]
        public bool ForceUpdate { get; set; } = false; // Nếu true, client sẽ tự động cập nhật khi ở homepage
    ...
    public class Proxy
    {
        ...
        // === Latency Check Fields ===
        [Display(Name = "Độ trễ (ms)")]
        public int? LatencyMs { get; set; } // null = chưa kiểm tra

        [Display(Name = "Lần cuối kiểm tra độ trễ (UTC)")]
        public DateTime? LastLatencyCheckUtc { get; set; }

        [Display(Name = "Trạng thái kiểm tra")]
        [StringLength(100)]
        public string? LatencyCheckStatus { get; set; } // "OK", "Timeout", "Error: xxx"
    ...
    public class SubtitleApiSetting
    {
        ...
        [Display(Name = "URL Server Chính (Callback)")]
        [StringLength(500)]
        public string? MainServerUrl { get; set; }
        
        [Display(Name = "Chu kỳ RPM (giây)")]
        public int RpmPeriodSeconds { get; set; } = 60; // Mặc định 60 giây = 1 phút
    ...
    /// <summary>
    /// Cài đặt Auto Backup - gửi email định kỳ
    /// </summary>
    public class BackupSetting
    {
        ...
        [Display(Name = "Bật Auto Backup")]
        public bool AutoBackupEnabled { get; set; } = true;

        [Display(Name = "Khoảng cách backup (giờ)")]
        public int BackupIntervalHours { get; set; } = 12;

        [EmailAddress]
        [StringLength(255)]
        [Display(Name = "Email nhận backup")]
        public string? NotificationEmail { get; set; } = "hoquangtruongctld4@gmail.com";

        [Display(Name = "Lần backup cuối")]
        public DateTime? LastBackupUtc { get; set; }

        [Display(Name = "Lần gửi email cuối")]
        public DateTime? LastEmailSentUtc { get; set; }

        [Display(Name = "Lần cập nhật")]
        public DateTime UpdatedAt { get; set; } = DateTime.UtcNow;
    }
