...
namespace SubPhim.Server.Services
{
    ...
    public class ProxyService
    {
        ...
        private readonly ProxyStatsQueueService? _statsQueue;
        
        // Configuration constants
        private const int MaxConsecutiveFailuresBeforeDisable = 5;
        private const int MaxFailureReasonLength = 500;
        private const int ProxyConnectTimeoutSeconds = 10;
        private const int HttpClientTimeoutMinutes = 3;
        ...
        private static readonly TimeSpan CacheDuration = TimeSpan.FromMinutes(5); // Tăng từ 1 lên 5 phút
        ...
        public ProxyService(IServiceProvider serviceProvider, ILogger<ProxyService> logger, ProxyStatsQueueService? statsQueue = null)
        {
        ...
        /// <summary>
        /// Get the next proxy, prioritizing by lowest latency (fastest response).
        /// Proxies with measured latency are preferred over unmeasured ones.
        /// </summary>
        /// <param name="excludeProxyIds">Optional set of proxy IDs to exclude (e.g., proxies that failed in current request)</param>
        /// <param name="preferFastest">If true, always pick the fastest proxy. If false, use weighted round-robin.</param>
        public async Task<Proxy?> GetNextProxyAsync(HashSet<int>? excludeProxyIds = null, bool preferFastest = true)
        {
            ...
            if (!proxies.Any())
            {
                // AUTO-RECOVERY: Nếu không còn proxy nào được bật, tự động bật lại các proxy có ít lỗi
            ...
            // === ƯU TIÊN PROXY THEO LATENCY ===
            // Sắp xếp: proxy có latency thấp nhất lên đầu, proxy chưa check ở cuối
            var sortedProxies = proxies
                .OrderBy(p => p.LatencyMs == null) // Có latency trước, chưa có latency sau
                .ThenBy(p => p.LatencyMs ?? int.MaxValue) // Trong nhóm có latency, sắp xếp tăng dần
                .ToList();

            if (preferFastest)
            {
                // Luôn chọn proxy nhanh nhất có sẵn
                var fastestProxy = sortedProxies.First();
                
                _logger.LogDebug("Selected fastest proxy {ProxyId}: {Host}:{Port} ({Type}) - Latency: {LatencyMs}ms", 
                    fastestProxy.Id, fastestProxy.Host, fastestProxy.Port, fastestProxy.Type, 
                    fastestProxy.LatencyMs?.ToString() ?? "N/A");
                
                return fastestProxy;
            }
            else
            {
                // Round-robin với ưu tiên proxy có latency thấp
                lock (_proxyLock)
                {
                    if (_proxyRoundRobinIndex >= sortedProxies.Count)
                        _proxyRoundRobinIndex = 0;
                    
                    var selectedProxy = sortedProxies[_proxyRoundRobinIndex];
                    _proxyRoundRobinIndex++;
                    
                    _logger.LogDebug("Selected proxy {ProxyId}: {Host}:{Port} ({Type}) - Latency: {LatencyMs}ms", 
                        selectedProxy.Id, selectedProxy.Host, selectedProxy.Port, selectedProxy.Type,
                        selectedProxy.LatencyMs?.ToString() ?? "N/A");
        ...
        /// <summary>
        /// Get the fastest proxy based on measured latency.
        /// Returns null if no proxies have been measured.
        /// </summary>
        public async Task<Proxy?> GetFastestProxyAsync(HashSet<int>? excludeProxyIds = null)
        {
            var proxies = await GetEnabledProxiesAsync();
            ...
            // Chỉ lấy proxy có latency đã đo
            var measuredProxies = proxies
                .Where(p => p.LatencyMs.HasValue && p.LatencyCheckStatus == "OK")
                .OrderBy(p => p.LatencyMs)
                .ToList();
            
            if (!measuredProxies.Any())
            {
                // Fallback về proxy bất kỳ nếu chưa có proxy nào được đo
                return proxies.FirstOrDefault();
            }
            
            return measuredProxies.First();
        }
        ...
        /// Uses fire-and-forget via ProxyStatsQueueService to reduce I/O.
        /// </summary>
        public void RecordProxySuccess(int proxyId)
        {
            if (_statsQueue != null)
            {
                // Fire-and-forget - không chờ database write
                _statsQueue.RecordSuccess(proxyId);
                _logger.LogDebug("Queued success record for proxy {ProxyId}", proxyId);
            }
            else
            {
                // Fallback nếu không có queue service
                _ = RecordProxySuccessDirectAsync(proxyId);
            }
        }
        
        /// <summary>
        /// [Legacy] Direct database write for proxy success - use RecordProxySuccess() instead.
        /// </summary>
        private async Task RecordProxySuccessDirectAsync(int proxyId)
        {
            try
            {
                await ExecuteWithRetryAsync(async () =>
            ...
            {
                _logger.LogError(ex, "Error recording proxy success for ID {ProxyId}", proxyId);
            }
        }
        
        /// <summary>
        /// [Obsolete - use RecordProxySuccess instead]
        /// </summary>
        [Obsolete("Use RecordProxySuccess() for fire-and-forget. This method is kept for compatibility.")]
        ...
        /// <summary>
        /// Record a proxy failure using fire-and-forget.
        /// For intermittent failures (HTML response, etc.), this increments failure count but doesn't immediately disable.
        /// </summary>
        public void RecordProxyFailure(int proxyId, string reason, bool isIntermittent = false, bool isTimeoutError = false)
        {
            if (_statsQueue != null)
            {
                // Fire-and-forget - không chờ database write
                _statsQueue.RecordFailure(proxyId, reason, isIntermittent, isTimeoutError);
                _logger.LogDebug("Queued failure record for proxy {ProxyId}: {Reason}", proxyId, 
                    reason?.Length > 50 ? reason.Substring(0, 50) + "..." : reason);
            }
            else
            {
                // Fallback nếu không có queue service
                _ = RecordProxyFailureDirectAsync(proxyId, reason, isIntermittent, isTimeoutError);
            }
        }
        
        /// <summary>
        /// [Legacy] Direct database write for proxy failure - use RecordProxyFailure() instead.
        /// </summary>
        private async Task RecordProxyFailureDirectAsync(int proxyId, string reason, bool isIntermittent, bool isTimeoutError)
        {
            try
            {
                await ExecuteWithRetryAsync(async () =>
                {
                    ...
                    if (proxy != null)
                    {
                        ...
                        int disableThreshold = isTimeoutError 
                            ? MaxConsecutiveFailuresBeforeDisable * 4
                            : (isIntermittent 
                                ? MaxConsecutiveFailuresBeforeDisable * 2 
                                : MaxConsecutiveFailuresBeforeDisable);
                        
                        if (proxy.FailureCount >= disableThreshold)
                        {
                            proxy.IsEnabled = false;
                            _logger.LogWarning("Proxy {ProxyId} disabled due to {FailureCount} failures", proxyId, proxy.FailureCount);
                            RefreshCache();
                        }
            ...
            {
                _logger.LogError(ex, "Error recording proxy failure for ID {ProxyId}", proxyId);
            }
        }
        
        /// <summary>
        /// [Obsolete - use RecordProxyFailure instead]
        /// </summary>
        [Obsolete("Use RecordProxyFailure() for fire-and-forget. This method is kept for compatibility.")]
        public async Task RecordProxyFailureAsync(int proxyId, string reason, bool isIntermittent = false, bool isTimeoutError = false)
        {
            RecordProxyFailure(proxyId, reason, isIntermittent, isTimeoutError);
            await Task.CompletedTask;
        }
