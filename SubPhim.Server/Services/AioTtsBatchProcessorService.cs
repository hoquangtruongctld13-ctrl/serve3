...
namespace SubPhim.Server.Services
{
    public class AioTtsBatchProcessorService : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                if (!_isProcessing)
                {
                    ...
                    catch (SqliteException ex) when (IsSqliteTransientError(ex))
                    {
                        // Log but don't crash - transient SQLite errors will be retried on next loop
                        _logger.LogWarning(ex, "Transient SQLite error in ProcessNextJobAsync. Will retry on next cycle.");
                        _isProcessing = false;
                    }
                    catch (Exception ex) when (ex is not OperationCanceledException)
                    {
                        // Log unexpected errors but don't crash the background service
                        _logger.LogError(ex, "Unexpected error in ProcessNextJobAsync. Service will continue.");
                        _isProcessing = false;
                    }
                }
        ...
        private static bool IsSqliteTransientError(SqliteException ex)
        {
            // SQLite error codes for transient errors:
            // 5 = SQLITE_BUSY (database is locked)
            // 6 = SQLITE_LOCKED (table is locked)
            return ex.SqliteErrorCode == 5 || ex.SqliteErrorCode == 6;
        }
        
        private async Task<T> ExecuteWithRetryAsync<T>(Func<Task<T>> operation, string operationName, CancellationToken stoppingToken)
        {
            for (int retry = 0; retry < MaxRetries; retry++)
            {
                ...
                catch (SqliteException ex) when (IsSqliteTransientError(ex))
                {
                    ...
                    {
                        _logger.LogWarning("SQLite transient error during {Operation} (attempt {Attempt}/{MaxRetries}): {Error}", operationName, retry + 1, MaxRetries, ex.Message);
                        await Task.Delay(RetryDelayMs * (retry + 1), stoppingToken);
                    }
                    else
                    {
                        _logger.LogError(ex, "SQLite transient error during {Operation} failed after {MaxRetries} attempts", operationName, MaxRetries);
                        throw;
                    }
                }
            }
            throw new InvalidOperationException($"Unexpected exit from retry loop during {operationName}");
        }
        
        private async Task ExecuteWithRetryAsync(Func<Task> operation, string operationName, CancellationToken stoppingToken)
        {
            await ExecuteWithRetryAsync(async () => { await operation(); return true; }, operationName, stoppingToken);
        }
        ...
        private async Task ProcessNextJobAsync(CancellationToken stoppingToken)
        {
            ...
            // Query with retry logic to handle SQLite transient errors
            var pendingJob = await ExecuteWithRetryAsync(
                () => context.AioTtsBatchJobs.AsNoTracking()
            ...
            // Query with retry logic to get job with User included
            var job = await ExecuteWithRetryAsync(
                () => context.AioTtsBatchJobs.Include(j => j.User).FirstOrDefaultAsync(j => j.Id == pendingJob.Id, stoppingToken),
                "loading job details",
                stoppingToken);
            ...
            try
            {
                ...
                await ExecuteWithRetryAsync(() => context.SaveChangesAsync(stoppingToken), "updating job status", stoppingToken);
                ...
                await ExecuteWithRetryAsync(() => context.SaveChangesAsync(stoppingToken), "updating user quota", stoppingToken);
                ...
                await ExecuteWithRetryAsync(() => context.SaveChangesAsync(stoppingToken), "saving processed lines", stoppingToken);
            ...
            finally
            {
                ...
                await ExecuteWithRetryAsync(() => context.SaveChangesAsync(stoppingToken), "saving final state", stoppingToken);
                _isProcessing = false;
            }
        }
